...
per_hub_init(cnodeid_t cnode)
{
...
#ifdef BRINGUP2
#if 0
	while ((REMOTE_HUB_L(nasid, IIO_IFDR)) != 0x2d2d)
		REMOTE_HUB_S(nasid, IIO_IFDR, 0xadad);
#endif
	printk("IIO_IFDR = 0x%x\n", REMOTE_HUB_L(nasid, IIO_IFDR));
	...
	REMOTE_HUB_S(nasid, IIO_IDBSS, 0x504);
	REMOTE_HUB_S(nasid, SH_DEBUG_SELECT, 0x0002DF6D74CF2C70);
#endif
